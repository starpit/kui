  ...
  it(`should split the terminal via command in the current tab and expect splitCount=${splitCount}`, async () => {
    try {
      ...
      const splitIdsBefore =
        where === undefined
          ? undefined
          : await this.app.client.$(Selectors.SPLITS).then(_ => _.getAttribute(Selectors.SPLIT_ID))
      if (where) {
      ...
      if (where !== undefined) {
        ...
        const splitIdsAfter = await this.app.client.$(Selectors.SPLITS).then(_ => _.getAttribute(Selectors.SPLIT_ID))
